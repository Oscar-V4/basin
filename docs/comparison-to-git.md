...
| `git log --graph` | `basin tui` Threads | A terminal cockpit for fork/merge lanes, checkpoint diffstats, and detail panels. |
...
Git's graph shows commits and refs. Basin's cockpit shows the context history that matters to an assistant: branch workrooms, checkpoint transitions, branch-vs-Canon proposals, atom provenance, and graph clusters generated by `basin graph`. It is a review surface over semantic state, not a transcript viewer.
