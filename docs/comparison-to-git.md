...
| Commit | Checkpoint | A *semantic state transition*, not every keystroke (or chat turn). |
...
A Git diff over a transcript is almost useless: the meaningful change is rarely a contiguous block of text, and most of the transcript is noise. Basin treats the transcript as **raw input**, distills it into atoms with status and provenance, and versions *those*. So you can ask "what has been decided, what's still open, and what did we explicitly reject?" — and get an answer without rereading the conversation.
