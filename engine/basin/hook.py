"""Claude Code hook entry. fail-soft: always exit 0, never write stdout.

Modes: session_start | precompact | session_end. The deterministic path (capture +
staging) runs immediately; the LLM worker is intentionally not invoked here (later,
detached). Reads the hook JSON on stdin (session_id / transcript_path / cwd).
"""
...
def run(mode: str) -> int:
    try:
        ...
        kind = "pre_compact" if mode == "precompact" else "session_end"
